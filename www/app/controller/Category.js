Ext.define('TouchStyle.controller.Category', {
	...
	console.log('********record is', record)
        ...
        if (isRoot) {
            record.set('label', 'PopShop For Etsy');
        }else{
			record.set('label', 'tester')
		}
    ...
    showProducts: function(record) {
        ...
        store.getProxy().setExtraParam('urlId', record.get('urlId'));
		store.getProxy().setUrl('http://openapi.etsy.com/v2/shops/' + record.get('urlId') + '/listings/active.js')
        store.load();
		...
		view = this.getProductsView({
			title: record.get('label'),
		});
		view.setStore(store);
		console.log('this.productsView', this.productsView);
		this.getMain().setActiveItem(view);
		console.log('record label', record.get('label'));
